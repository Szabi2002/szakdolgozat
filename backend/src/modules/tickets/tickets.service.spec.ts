  ...
  describe('sendEmail', () => {
    it('should send email successfully', async () => {
      ...
      mockQrCodeService.createQRCodeData.mockReturnValue({
        ...
        userId,
        ticketType: 'single',
        validUntil: mockTicket.valid_until,
        signature: 'abc123',
      });
      ...
      const result = await service.sendEmail(ticketId, userId);

      expect(result.success).toBe(true);
      expect(mockUsersService.findById).toHaveBeenCalledWith(userId);
      expect(mockEmailService.sendTicketEmail).toHaveBeenCalledWith(
        mockTicket,
        mockUser,
        mockQrBuffer,
      );

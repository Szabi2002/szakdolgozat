...
WHERE u.role = 'admin'
GROUP BY u.id, u.email, u.name, u.display_name;
...
SELECT
  ...
  ma.admin_id,
  u.email as admin_email,
  CONCAT(u.name, ' ', u.display_name) as admin_name,
  ma.action_type,
  ma.target_type,
  ma.target_id,
  ma.previous_value,
...
WHERE u.role = 'admin'
GROUP BY u.id, u.email, u.name, u.display_name, at.avg_seconds_between_actions;

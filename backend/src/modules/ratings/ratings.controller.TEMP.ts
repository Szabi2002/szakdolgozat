...
export class RatingsController {
  ...
  @UseGuards(AdminGuard)
  @ApiOperation({
    summary: 'Get all ratings with filter (Admin only)',
    description: 'Get all ratings with optional status filter (pending, approved, rejected). Admin only.',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  async getAllRatings(@Query('status') status?: string): Promise<RatingResponseDto[]> {
    return this.ratingsService.getAllRatings(status);
  }
